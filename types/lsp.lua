...
--- @param index any #(`integer`) Optional index of the edit, if from
...
--- @param row any #(`integer`) 0-indexed line
--- @param col any #(`integer`) 0-indexed byte offset in line
...
--- @param options any #(`table?`) Optional table which holds the following
---                optional fields:
...
--- @param input any #(`lsp.MarkedString|lsp.MarkedString[]|lsp.MarkupContent`)
--- @param contents any #(`table?`) List of strings to extend with converted lines.
---                 Defaults to {}.
...
--- @param ft any #(`string?`) filetype that will be use as the `lang`
---                       for the label markdown code block
--- @param triggers any #(`table?`) list of trigger characters from the lsp
---                       server. used to better determine parameter offsets
--- @return any #(`table?`) table list of lines of converted markdown.
--- @return any #(`table?`) table of active hl
function vim.lsp.convert_signature_help_to_markdown_lines(signature_help, ft, triggers) end

-- Jumps to the declaration of the symbol under the cursor.
--- @param options any #(`table?`) additional options
...
--- @param lenses any #(`lsp.CodeLens[]?`) lenses to display
...
--- @param bufnr any #(`integer?`) filter by buffer. All buffers if nil, current
---              buffer if 0
...
--- @param filter any #(`vim.lsp.inlay_hint.get.filter?`) Optional filters
---               |kwargs|:
...
--- @param bufnr any #(`integer?`) Buffer number (0 for current buffer, default)
--- @param row any #(`integer?`) Position row (default cursor position)
--- @param col any #(`integer?`) Position column (default cursor position)
--- @return any #(`table?`) List of tokens at position. Each token has the following
---     fields:
...
--- @param bufnr any #(`integer?`) Buffer handle, defaults to current
...
--- @return any #(`lsp.Diagnostic`) Get the diagnostic namespace associated with an LSP
---     client |vim.diagnostic| for diagnostics
...
--- @param start_pos any #(`integer[]?`) {row,col} mark-indexed position.
---                        Defaults to the start of the last visual selection.
--- @param end_pos any #(`integer[]?`) {row,col} mark-indexed position.
---                        Defaults to the end of the last visual selection.
--- @param bufnr any #(`integer?`) buffer handle or 0 for current,
---                        defaults to current
--- @param offset_encoding any #(`"utf-8"|"utf-16"|"utf-32"?`) defaults to
---                        `offset_encoding` of first client of `bufnr`
--- @return any #(`table`) { textDocument = { uri = `current_file_uri` }, range = {
...
-- cursor position.
--- @param window any #(`integer?`) window handle or 0 for current,
---                        defaults to current
...
--- @param opts any #(`table`) with optional fields (additional keys are
...
--- @return any #(`integer?`) buffer id of float window
--- @return any #(`integer?`) window id of float window
...
--- @param server_capabilities any #(`table`) Table of capabilities supported by
---                            the server
--- @return any #(`lsp.ServerCapabilities?`) Normalized table of capabilities
...
--- @param lenses any #(`lsp.CodeLens[]?`) lenses to store
--- @param bufnr any #(`integer`)
--- @param client_id any #(`integer`)
